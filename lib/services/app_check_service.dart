...
class AppCheckService {
  ...
  Future<void> initialize() async {
    try {
      ...
      _tokenSubscription = FirebaseAppCheck.instance.onTokenChange.listen(
        _handleTokenRefresh,
        onError: (error) {
          _logger.warning('App Check token refresh error: $error');
        },
      );
      ...
      if (kDebugMode) {
        ...
        if (token != null) {
          ...
          _logger.info('Initial debug token obtained');
        }
      }

      _logger.info('App Check service initialized successfully');
    } catch (e) {
      _logger.severe('Failed to initialize App Check service: $e');
      rethrow;
    }
  }
  ...
  Future<void> _handleTokenRefresh(String? token) async {
    if (token != null) {
      await _updateTokenTimestamp();
      _logger.info('App Check token refreshed automatically');
    }
  }
  ...
  Future<void> _updateTokenTimestamp() async {
    try {
      ...
      _logger.warning('Failed to update token timestamp: $e');
    }
  }
  ...
  Future<bool> validateToken() async {
    try {
      ...
      if (needsRefresh) {
        _logger.info('Token refresh needed, requesting new token');
  ...
  Future<bool> _needsTokenRefresh() async {
    try {
      ...
      _logger.warning('Error checking token refresh need: $e');
      return true;
    }
  }
  ...
  Future<Map<String, dynamic>> getTokenStatus() async {
    try {
      ...
      _logger.severe('Error getting token status: $e');
      return {
        'hasValidToken': false,
        'lastRefresh': null,
        'needsRefresh': true,
  ...
  void dispose() {
    ...
    _logger.info('App Check service disposed');
  }
}
